...
// MuLawDecode is ai generated
func MuLawDecode(muLawByte byte) int16 {
	// Expand to 16 bits
	// µ-law bytes are 8 bits, where the first bit is the sign bit
	signBit := int16(muLawByte & 0x80)
	muLawByte = ^muLawByte

	// Decompress according to µ-law algorithm
	t := int16((muLawByte & 0x0F) << 3)
	seg := int16((muLawByte & 0x70) >> 4)
	if seg != 0 {
		t |= (0x1 << (seg + 2)) - 1
	}

	// Add bias and scale factor, then negate to get final value
	t = (t + 33) << 3
	if signBit != 0 {
		t = -t
	}
...
// MuLawDecode2 decodes a single μ-law byte to a 16-bit PCM value.
// TODO(Cinjon): The above two are not the same. Test which one is better.
func MuLawDecode2(muByte byte) int16 {
	// Invert the bits
	muByte = ^muByte
	...
	// Convert the quantized value into a linear PCM value
	sample := int16((muByte << 3) + 0x84)
	sample <<= 3

	// Expand the exponential value to linear
	sample = (sample - 33) << 1
	sample += 33
